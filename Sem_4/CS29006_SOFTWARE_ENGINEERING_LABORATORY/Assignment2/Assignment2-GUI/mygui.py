...
class CommunicationModule(PrimaryDisplay):
    def __init__(self):
        ...
        self.content = Text(self.inputframe_text, font=Font(family="Times New Roman", size=10), wrap=WORD)
        ...
        self.remove_image_button = Button(self.inputframe_image, text='REMOVE\nIMAGE', command=self.remove_image)
        self.image_info = Text(self.root, font=Font(family="Times New Roman", size=10), state=DISABLED)
    ...
    def pack(self):
        ...
        self.sendto_label.grid(row=10, column=0, pady=10)
        self.sendto_list.place(x=100, y=32)
...
class MessagesDisplay(UserInfoDisplay):
    def __init__(self):
        ...
        self.button = Button(self.topframe, text='REFRESH', command=self.refresh)
...
if __name__ == '__main__':
    ...
    root.resizable(0, 0)
    button = Button(root, text='QUIT APPLICATION', bg='gray', command=QuitApp, width=15, height=3)
    button.pack(pady=10)
