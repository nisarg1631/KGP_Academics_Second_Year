...
int main()
{
	...
	if(check==1)
	{
		...
		cout<<"Path not found\n";
	cout<<"The level order traversal of the given grid is \n";
	cout<<row1<<" "<<col1<<endl;
	for(int i=0;i<n;i++)
	{
		for(int j=0;j<n;j++)
		{
			if(cells[i][j]->parent!=cells[i][j]&&((cells[i][j]->parent)->data==1))
			{
				//cout<<i<<" "<<j<<" "<<((cells[i][j]->parent)->point).x<<" "<<((cells[i][j]->parent)->point).y<<endl;
				cout<<i<<" "<<j<<endl;
			}
		}
	}
