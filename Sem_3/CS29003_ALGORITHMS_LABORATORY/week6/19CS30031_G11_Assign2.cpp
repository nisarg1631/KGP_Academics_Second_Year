...
//utility class with functions to perform the merge subroutine
/*class merge{
	public:
		interval* point_arr;
		int len;
		...
		merge(int maxlen){
			point_arr=new interval[maxlen];
            len=maxlen;
		}
		
		//utility function to merge two arrays
		merge* merge_sort(merge *data){
			int lenl, lenr;
			
			lenl=len;
			lenr=data->len;
			merge *ans=new merge(lenl+lenr);
			
			int it, itl, itr;
			for(it=0, itl=0, itr=0; itl<lenl && itr<lenr;){
...
merge* sort_array(interval arr[], int l, int r){
	if(l==r){
		merge *ans=new merge(1);
		ans->point_arr[0].start=arr[l].start;
        ans->point_arr[0].end=arr[l].end;
		return ans;
	}

	int mid=(l+r)/2;
	
	merge *left=sort_array(arr,l,mid);
	merge *right=sort_array(arr,mid+1,r);
	merge *ans=left->merge_sort(right);
